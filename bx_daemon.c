#ifndef LINT
static char sccs_bx_daemon_id[] = "@(#)bx_daemon.c Version 1.1 Date 00/02/21 Hour 12:06:21";
#endif
/*---------------------------------------------------------------------------

	bx_daemon.c:  funzione per la gestione dell'interfacciamento
		      tra il S.I.A. e il Check-in/Check-out da PABX

	Parametri in input - tty, speed, parity, databit, stopbit, N. pabx
...
struct dls_msg dls_msgstruct;
    ...
    if(argc < 7) {
       fprintf(stderr,"Usage: %s tty, speed, parity, databit, stopbit, N. pabx\n",argv[0]);
       exit(1);
       }

    n_pabx = atoi(argv[6]);
    if((dls_shmid = shmget((key_t)SHMKEY,sizeof(sia_status),0666)) == ERROR) {
   ...
   /*
    * Accedo coda di messaggi
    */

    if((dls_qr = msgget((key_t)69,0660)) == ERROR) {
